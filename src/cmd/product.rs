...
pub fn run(cmd: &ProductAction, config: &Config, _format: OutputFormat, dry_run: bool) {
    ...
    rt.block_on(async {
        ...
        match cmd {
            ...
            ProductAction::List => {
                if dry_run {
                    safe_println("[DRY-RUN] Would call ProductApi::list()");
            ...
            ProductAction::Get { id } => {
                if dry_run {
                    safe_println("[DRY-RUN] Would call ProductApi::get()");
            ...
            ProductAction::Create { name, code, desc } => {
                if dry_run {
                    safe_println("[DRY-RUN] Would call ProductApi::create()");
                    println!("  URL: {}/api.php/v1/products", config.url);
                    println!("  name: {}", name);
                    return;
                }
            ...
            ProductAction::Update {
                ...
                if dry_run {
                    safe_println("[DRY-RUN] Would call ProductApi::update()");
            ...
            ProductAction::Delete { id } => {
                if dry_run {
                    safe_println("[DRY-RUN] Would call ProductApi::delete()");
                    println!("  URL: {}/api.php/v1/products/{}", config.url, id);
                    return;
                }
