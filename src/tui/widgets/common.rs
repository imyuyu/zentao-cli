...
pub fn render_bug_list(frame: &mut Frame, area: Rect, items: &[ListItem], selected: usize) {
    ...
    // Footer with help
    let footer = Paragraph::new(Text::from(vec![Line::from(Span::raw(format!(
        "Selected: {}",
        selected + 1
    )))]));
...
pub fn render_bug_detail(frame: &mut Frame, area: Rect, bug: &crate::api::Bug) {
    ...
    let details = Paragraph::new(Text::from(vec![
        Line::from(vec![Span::raw("Status: "), Span::raw(bug.status.as_str())]),
        Line::from(vec![
            Span::raw("Severity: "),
    ...
    let steps = Paragraph::new(Text::from(vec![Line::from(Span::raw(
        bug.steps.as_deref().unwrap_or("No reproduction steps"),
    ))]))
